...
export async function POST(req: NextRequest) {
  try {
    const sql = getSql();
    const { kod, cihazId, surum, sadeceOttoPlus } = await req.json();
    if (!kod || !cihazId) return NextResponse.json({ ok:false, hata:'eksik' }, { status:400 });
    const rows = await sql`SELECT kod, durum, cihaz_id, iptal, tip FROM otto_kodlar WHERE kod=${kod}`;
    if (rows.length === 0) return NextResponse.json({ ok:false, hata:'gecersiz' });
    const k = rows[0];
    const tip: 'otto' | 'otto+' = k.tip === 'otto+' ? 'otto+' : 'otto';
    if (sadeceOttoPlus === true && tip !== 'otto+') {
      return NextResponse.json({ ok:false, hata:'yanlis_tip', mesaj:'Bu kod Otto+ için geçerli değil. Otto+ kodu gerekiyor.' });
    }
